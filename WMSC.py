...
async def _(bot: Bot, event: MessageEvent, arg: Message = CommandArg()):
    ...
    # 判断上一场是否结束
    if checkGroup(gid):
        await jiesuan.finish(f"请先开场、开局后才能结算")

    #如果玩家不在列表里
    if checkPlayer(gid, uid):
        await jiesuan.finish(f"无关人员不要捣乱")
...
async def end(gid: int, text: str):
    ...
    for player in Ginfo[gid]["playerInfo"]:
        ...
        if player["niubi"] > 0:
            text += f"，达成亡命神抽 {player['niubi']} 次，额外奖励 {player['niubi'] * 500} 金币\n"
            await BagUser.add_gold(player["uid"], gid, player['niubi'] * 500)
        else:
            text += f"\n"
    ...
    for player in Ginfo[gid]["playerInfo"]:
        if player["score"] == maxScore:
            text += f" {player['uname']}"
            await BagUser.add_gold(player["uid"], gid, 100)
    text += f"，奖励100金币，恭喜！"
